...
def check_cutout(hdulist, center_pix, center_world, cutout_size, ecube, eps=1.0e-7):
    ...
    for i, v in enumerate(world_coords):
        ...
        if use_factory:
            ...
            cutout_maker.cube_cut(
                cube_file, coord, csize[i], target_pixel_file=cutlist[i], output_path=tmpdir, verbose=False
            )
        else:
            cube_cut(cube_file, coord, csize[i], target_pixel_file=cutlist[i], output_path=tmpdir, verbose=False)
