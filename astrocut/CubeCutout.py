...
class CubeCutout(Cutout):
    def __init__(self, input_files, coordinates, product="SPOC",cutout_size = 25, fill_value = np.nan, memory_only = False, output_dir = '.', 
                 verbose = True, threads: Union[int, Literal["auto"]] = 1,target_pixel_file=None):
        ...
        self.target_pixel_file = target_pixel_file
    ...
    def _parse_table_info(self, table_data, verbose):
        """
    ...
    def _load_data(self):
        ...
        self.fits_options: Dict[str, Any] = {"mode": "denywrite","lazy_load_hdus": True}

        if self.input_files.startswith("s3://"):
            self.fits_options["use_fsspec"] = True
            self.fits_options["fsspec_kwargs"] = {"default_block_size": 10_000, "anon": True}
            self.cube_data_prop = "section"
        else:
            self.fits_options["memmap"] = True
            self.cube_data_prop = "data"
            self.threads = 1

        with fits.open(self.input_files, **self.fits_options) as cube:
            self._parse_table_info(cube[2].data, self.verbose)

            if isinstance(self.coordinates, SkyCoord):
                self.center_coord = self.coordinates
            else:
                self.center_coord = SkyCoord(self.coordinates, unit='deg')

            log.debug("Cutout center coordinate: %s, %s", self.center_coord.ra.deg, self.center_coord.dec.deg)

            # Making size into an array [ny, nx]
            if np.isscalar(self.cutout_size):
                self.cutout_size = np.repeat(self.cutout_size, 2)

            if isinstance(self.cutout_size, u.Quantity):
                self.cutout_size = np.atleast_1d(self.cutout_size)
                if len(self.cutout_size) == 1:
                    self.cutout_size = np.repeat(self.cutout_size, 2)

            if len(self.cutout_size) > 2:
                warnings.warn("Too many dimensions in cutout size, only the first two will be used.",
                              InputWarning)
                self.cutout_size = self.cutout_size[:2]

            self._get_cutout_limits(self.cutout_size)
    ...
    def cutout(self):
        ...
        _handle_verbose(self.verbose)
        self.product = self.product.upper()
                
        # load in data (THIS DOESN"T REALLY "LOAD" THE DATA IT REALLY JUST INITIALIZES IT)
        # This is also a little weird as it currently opens the Cube file _twice_ once to load and once and initialize, then again to do the cutout
        self._load_data()
        ...
        with fits.open(self.input_files, **self.fits_options) as cube:
            ...
            #get cutout
            img_cutout, uncert_cutout, aperture = self._get_cutout(getattr(cube[1], self.cube_data_prop), threads=self.threads, verbose=self.verbose)
            ...
            self._write_cutout(tpf_object)

    def _write_cutout(self,tpf_object):
        if not self.target_pixel_file:
            _, flename = os.path.split(self.input_files)

            width = self.cutout_lims[0, 1]-self.cutout_lims[0, 0]
            height = self.cutout_lims[1, 1]-self.cutout_lims[1, 0]
            self.target_pixel_file = "{}_{:7f}_{:7f}_{}x{}_astrocut.fits".format(flename.rstrip('.fits').rstrip("-cube"),
                                                                                        self.center_coord.ra.value,
                                                                                        self.center_coord.dec.value,
                                                                                        width,
                                                                                        height)
                
        self.target_pixel_file = os.path.join(self.output_dir, self.target_pixel_file)
                

        log.debug("Target pixel file: %s", self.target_pixel_file)
        ...
        # Write the TPF
        tpf_object.writeto(self.target_pixel_file, overwrite=True, checksum=True)

        return self.target_pixel_file